...
function FlowBadge({ flow }: { flow: TradeFlow }) {
  const map: Record<TradeFlow, { glyph: string; label: string; cls: string; title: string }> = {
    ...
    "swap-in":  { glyph: "↘", label: "SWAP IN",  cls: "border-[var(--accent-line)] bg-[var(--accent-tint)] text-[var(--accent-soft)]",                                                                              title: "Swap in · received this player share in a player ↔ player swap (5% fee). The matching swap-out leg appears as a separate row when the other side is an NFL token; cross-sport swaps (NFL ↔ Soccer) only show the NFL leg." },
    "swap-out": { glyph: "↗", label: "SWAP OUT", cls: "border-[var(--accent-line)] bg-[var(--accent-tint)] text-[var(--accent-soft)]",                                                                              title: "Swap out · gave up this player share in a player ↔ player swap (5% fee). The matching swap-in leg appears as a separate row when the other side is an NFL token; cross-sport swaps (NFL ↔ Soccer) only show the NFL leg." },
  };
  const m = map[flow];
  return (
    <span
